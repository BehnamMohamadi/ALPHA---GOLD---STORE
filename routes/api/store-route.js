...
router.put("/admin/settings", validate(Joi.object({ heroTitle: Joi.string().trim().min(2).max(100), heroSubtitle: Joi.string().trim().max(240).allow(""), phone: Joi.string().max(30).allow(""), email: Joi.string().email().allow(""), address: Joi.string().max(500).allow(""), instagram: Joi.string().uri({ scheme: ["https"] }).allow(""), salesEnabled: Joi.boolean(), maxRateAgeMinutes: Joi.number().integer().min(1).max(10080), otpTtlSeconds: Joi.number().integer().min(60).max(300), otpResendSeconds: Joi.number().integer().min(30).max(180) }).min(1).unknown(false)), wrap(async (req, res) => {
